...
public final class zabq implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, zau {
    ...
    private final Feature zaC(Feature[] featureArr) {
        if (featureArr != null && featureArr.length != 0) {
            ...
            di diVar = new di(availableFeatures.length);
            for (Feature feature : availableFeatures) {
                diVar.put(feature.getName(), Long.valueOf(feature.getVersion()));
            }
            for (Feature feature2 : featureArr) {
                Long l = (Long) diVar.get(feature2.getName());
    ...
    private final void zaF(Status status, Exception exc, boolean z) {
        ...
        if ((status == null) == (exc == null)) {
            k21.f("Status XOR exception should be null");
            return;
        }
    ...
    public final void zaH() {
        ...
        while (it.hasNext()) {
            ...
            if (zaC(zaciVar.zaa.getRequiredFeatures()) != null) {
                ...
                try {
                    zaciVar.zaa.registerListener(this.zac, new pw3());
    ...
    public final void zav() {
        ...
        for (ListenerHolder.ListenerKey listenerKey : (ListenerHolder.ListenerKey[]) this.zag.keySet().toArray(new ListenerHolder.ListenerKey[0])) {
            zap(new zah(listenerKey, new pw3()));
        }
