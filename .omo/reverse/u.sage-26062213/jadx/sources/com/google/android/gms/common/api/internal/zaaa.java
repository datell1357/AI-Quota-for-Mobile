...
final class zaaa implements zaca {
    ...
    private zaaa(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, Map map2, ClientSettings clientSettings, Api.AbstractClientBuilder abstractClientBuilder, Api.Client client, ArrayList arrayList, ArrayList arrayList2, Map map3, Map map4) {
        ...
        while (it.hasNext()) {
            diVar.put((Api.AnyClientKey) it.next(), this.zad);
        }
        Iterator it2 = map.keySet().iterator();
        while (it2.hasNext()) {
            diVar.put((Api.AnyClientKey) it2.next(), this.zae);
        }
        this.zaf = Collections.unmodifiableMap(diVar);
    }
    ...
    public static zaaa zag(Context context, zabe zabeVar, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map map, ClientSettings clientSettings, Map map2, Api.AbstractClientBuilder abstractClientBuilder, ArrayList arrayList) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            if (client2.requiresSignIn()) {
                diVar.put((Api.AnyClientKey) entry.getKey(), client2);
            } else {
                diVar2.put((Api.AnyClientKey) entry.getKey(), client2);
            }
        }
        Preconditions.checkState(!diVar.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        di diVar3 = new di(0);
        di diVar4 = new di(0);
        for (Api api : map2.keySet()) {
            Api.AnyClientKey anyClientKeyZab = api.zab();
            if (diVar.containsKey(anyClientKeyZab)) {
                diVar3.put(api, (Boolean) map2.get(api));
            } else {
                if (!diVar2.containsKey(anyClientKeyZab)) {
                    k21.n("Each API in the isOptionalMap must have a corresponding client in the clients map.");
                    return null;
                }
                diVar4.put(api, (Boolean) map2.get(api));
            }
        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (diVar3.containsKey(zatVar.zaa)) {
                arrayList2.add(zatVar);
            } else {
                if (!diVar4.containsKey(zatVar.zaa)) {
                    k21.n("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
                    return null;
                }
                arrayList3.add(zatVar);
            }
        }
        return new zaaa(context, zabeVar, lock, looper, googleApiAvailabilityLight, diVar, diVar2, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, diVar3, diVar4);
    }
    ...
    private final PendingIntent zaz() {
        ...
        return PendingIntent.getActivity(this.zaa, System.identityHashCode(this.zab), client.getSignInIntent(), ej4.a | 134217728);
    }
    ...
    public final void zau() {
        ...
        try {
            ...
            if (zZax) {
                new fj4(this.zac).post(new zav(this));
            } else {
                zaB();
            }
            this.zam.unlock();
