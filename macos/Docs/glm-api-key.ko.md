...
- 같은 연결에는 같은 키·지역·범위만 재연결할 수 있다. 키 또는 범위 변경을 동의받아 새 연결로 전환하는 사용자 기능은 아직 남아 있다. 현재 코어의 연결 해제는 신원 결합 정보를 보존하므로, 단순 연결 해제로 다른 키를 사용할 수 있다고 안내하지 않는다.
...
실제 Global/CN 개인·팀 키의 응답, 실제 API 키의 Keychain 재실행 복원, 실제 429·만료·미구독, 장기 수집, GLM 웹 로그인, 키/범위 교체 및 기존 자격 증명 정리의 영구 재시도는 남아 있다. API 키 단계의 합성/화면 검증으로 이 항목들을 통과 처리하지 않는다.
