...
2026-09-17 기준. 선택한 OAuth 토큰으로 Google 신원·프로젝트·모델별 쿼터를 확인하는 수집기를 앱 registry에 연결했다. **후속 단계에서 [로그인 UI와 앱 소유 OAuth 발급/갱신](antigravity-oauth.ko.md)을 구현했다.** 등록 Desktop client·실계정 수집·로컬 앱/CLI 연결은 아직 검증 대기이며, 기본 빌드의 로그인 버튼은 비활성 상태다.
