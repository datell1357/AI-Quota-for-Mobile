...
| 2. 수집·저장 | 계정·SQLite·알림·표시 스냅샷·60초 scheduler, Keychain·격리 WebKit·로그인 교체, Claude/Codex/Grok 웹 수집 및 응답 쿠키 갱신, GLM API 키 수집, DB 교체 후 표시 파일 복구. 코어 32/인증 16/수집 55개 테스트 통과 | 나머지 어댑터, 제공자별 인증/실계정 연동, 실제 전송·타이머 계측, 실패 정리의 영구 재시도 |
| 3. 사용자 기능 | 네이티브 앱·대시보드·메뉴 막대·온보딩·계정 편집·설정·Claude/Codex/Grok 로그인·GLM API 키 연결 화면 구현. 실제 UI에서 미로그인/잘못된 입력 차단·취소·다시 열기 확인 | 나머지 로그인 UI, GLM 웹 로그인·키/범위 교체, 실제 인증 성공, 메뉴 막대 팝오버 실제 클릭, 알림 전달/거부·자동 시작 검증 |
...
- Claude/Codex/Grok 로그인 화면/원격 신원 검증을 연결했다. 연결 해제 후 이전 credential/profile의 영구 정리 재시도, 나머지 제공자 어댑터, 위젯 확장의 실제 OS 검증과 패널의 장기 계측은 계속 남아 있다.
...
- 실제 계정의 Global/CN 개인·팀 수집, API 키 재실행 복원, GLM 웹 로그인, 키/범위 교체 사용자 기능, 장기 수집은 남아 있다. 코어 32개·인증 16개 테스트는 이 단계에서 재실행하지 않았으며 해당 소스는 변경하지 않았다.
