...
| 2. 수집·저장 | 계정·SQLite·알림·표시 스냅샷·60초 scheduler, Keychain·격리 WebKit·로그인 교체 기반, Claude/Codex 웹 수집 연결. 코어 19/인증 10/수집 27개 테스트 통과 | 나머지 어댑터, 제공자별 인증/실계정 연동, 실제 전송·타이머 계측, 실패 정리의 영구 재시도 |
...
- 수집 패키지 Swift Testing 27개 테스트 통과. 코어 19개, 인증 10개와 별도 패키지다. Codex 단계에서 수집 패키지를 재실행했고 변경 없는 코어/인증 패키지는 앞 단계의 통과 결과다.
