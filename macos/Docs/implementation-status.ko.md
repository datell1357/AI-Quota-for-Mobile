...
## Kiro 웹 로그인·크레딧 수집 검증

- [Kiro 웹 연결 계약](kiro-web-login.ko.md)에 배포 웹 클라이언트·Android CBOR 출처, 신원·단위·만료와 미검증 경계를 기록했다. 격리된 Google·GitHub·Builder ID 개인 계정의 userId를 사용량 수집 전후와 응답 안에서 대조한다. 조직 프로필은 ARN/지역 결합이 구현될 때까지 명시적으로 거부한다.
- 제한된 데이터 전용 CBOR 파서와 두 읽기 전용 POST operation을 연결했다. 월간 크레딧·활성 체험·보너스·선불·초과 사용량/요금을 분리한다. 실제 웹 UI가 쓰는 precision 수치를 보존하며 CLI 계산식과 혼합하지 않는다.
- CREDIT sentinel/누락 상한에서도 알려진 사용량은 유지하고 잔여율·무제한을 만들지 않는다. 코어 `.unknown.used`와 선택 필드 `expiresAt`을 추가했으며 이전 JSON을 읽는다. expiry를 reset과 분리해 만료만으로 리셋 알림이 생기지 않는다. 구버전 앱으로의 다운그레이드 호환성은 미검증이다.
- Kiro 14개를 포함한 수집 103개와 GLM 번들 실행, 코어 46개 테스트가 통과했다. Android 743바이트 캡처와 두 정규화 입력, 모든 절단 위치·자원 상한·정밀도/단위·신원/조직/HTTP 오류·취소, 인증 대역 → 실제 coordinator/SQLite/JSON consumer와 오류 후 이전 값/다른 계정 보존을 확인했다.
- 독립 네이티브 위젯 probe의 33개 렌더링이 통과했다. Kiro 한·영 small/medium/large를 추가했으며 작은 크기에서 사용량 문구가 잘리던 문제를 수정하고 확인했다. 실제 Debug 앱에서도 한·영 320 credits 사용/상한 미확인과 보너스 75%/만료 라벨을 검증했다. 자동 수집을 끈 합성 데이터 검사다.
...
## Antigravity 브라우저 OAuth·토큰 갱신 검증

- [Antigravity OAuth 계약](antigravity-oauth.ko.md)에 시스템 브라우저, Desktop client 설정, PKCE/state·loopback 수신기·scope·저장·갱신과 남은 경계를 기록했다. 등록 Client ID가 없으면 한영 연결 안내와 비활성 로그인 버튼을 표시한다. 실제 client 값은 설정하지 않았다.
- 수신기는 127.0.0.1의 임시 포트에만 바인딩한다. state·Host·경로·메서드·중복/본문/요청 크기를 확인하고, 전체 5분·개별 연결 5초 제한과 완료/취소 시 종료를 구현했다. 잘못된 콜백이 정상 시도를 소비하지 않으며 코드는 한 번만 전달된다.
- Google 숫자 ID·Antigravity 프로젝트·쿼터를 검증한 초기 토큰만 저장한다. 수집 전 앱 소유 만료 토큰을 같은 client로 갱신하고, 새 identity/프로젝트 확인 후 새 lease로 수집한다. 갱신 응답에서 생략된 refresh token·기존 절대 만료는 보존한다. 서버 회전과 로컬 저장의 원격 원자성은 보장하지 않는다.
- 인증 37개(신규 6개), 수집 175개(신규 9개 함수와 매개변수 사례) 테스트가 통과했다. 실제 NWListener↔URLSession 콜백 → 합성 Google 전송 → 실제 로그인·수집 coordinator → SQLite → 표시 consumer와 DB 재열기를 확인했다. 원래 계정·프로젝트와 다른 응답, 잘못된 token/scope/HTTP, 만료 refresh, 취소/시간 초과와 실패 후 이전 사용량/다른 계정 보존을 검사했다.
...
## Claude Code Keychain·파일 연결 검증

- [Claude Code 연결 계약](claude-code-connection.ko.md)에 공식 문서·고정 소스·선택 범위와 검증 경계를 기록했다. 지정한 Keychain service/account 또는 `.credentials.json`만 읽으며 저장 경로 간 자동 대체나 원본 토큰 갱신은 하지 않는다.
- 같은 Bearer로 서버 profile → usage → profile을 읽어 UUID 사용자/조직과 구독 상품을 확인한다. 로컬 이메일·플랜을 신원 근거로 사용하지 않는다. 기존 쿼터 decoder의 소수 값·단위는 유지하고 OAuth 지표 출처를 구분했다.
- Keychain 조회는 정확히 일치한 항목 하나의 persistent reference를 사용한다. 실제 probe의 `errSecParam(-50)`을 재현해 여러 비밀 데이터를 한 번에 요청하던 조합을 수정했고, 합성 항목의 읽기·원본 보존·다른 계정 대체 없음이 통과했다. 배경 작업은 접근 창을 띄우지 않는다.
- schema 1에 선택적 외부 Keychain 참조를 추가하고 기존 파일 연결 기록 호환성을 검증했다. 신원과 선택 위치만 저장하며 access/refresh token은 복사 저장하지 않는다. 수집 도중 원본 변경·계정 전환·만료·잠금·유실·취소·HTTP 오류에서 이전 값과 다른 계정을 보존한다.
- 인증 42개·수집 189개 및 기존 GLM 리소스 실행이 통과했다. 두 합성 파일/Keychain 계정 → 로그인/수집 coordinator → SQLite → 표시 consumer/재열기를 확인했다. 신규 테스트의 Swift 동시성 매크로 오류를 수정한 뒤 최종 전체 회귀를 통과했다.
