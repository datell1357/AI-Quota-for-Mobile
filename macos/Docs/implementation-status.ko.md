...
| 2. 수집·저장 | 계정·SQLite·알림·표시 스냅샷·60초 scheduler, Keychain·격리 WebKit·로그인 교체, Claude/Codex/Grok 웹 수집 및 응답 쿠키 갱신, DB 교체 후 표시 파일 복구. 코어 32/인증 16/수집 39개 테스트 통과 | 나머지 어댑터, 제공자별 인증/실계정 연동, 실제 전송·타이머 계측, 실패 정리의 영구 재시도 |
...
| 4. 위젯·패널 | 3종/6개 WidgetKit kind와 계정별 구성·딥링크 구현, 실제 View 렌더링·번들 검사 통과 | 서명된 공유/갤러리·독립 인스턴스 실측, 접근성/크기·설정 동기화, 고정 패널 |
...
- Claude/Codex/Grok 로그인 화면/원격 신원 검증을 연결했다. 연결 해제 후 이전 credential/profile의 영구 정리 재시도, 나머지 제공자 어댑터, 위젯 확장의 실제 OS 검증·고정 패널은 계속 남아 있다.
...
- 독립 렌더러·메타데이터 검사는 실제 WidgetKit 갤러리의 등록/편집/두 인스턴스 보존/클릭/접근성 실측을 대신하지 않는다. 서명된 App Group, 호스트 언어·테마 전달, 고정 패널과 장기 계측은 남아 있다. 단계 로그는 `artifacts/macos-20260917-widgets/`, 렌더링은 `artifacts/macos-widget-probes/`에 보관한다.
