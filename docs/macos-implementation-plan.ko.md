...
작성일: 2026-09-16. 검토 갱신: 2026-09-17. 상태: 구현 목표와 검증 기준. 이번 계획 검토에서는 소스·기록·공식 문서를 확인했으며 앱 빌드·실계정 로그인·장기 수집 시험을 새로 수행하지 않았다.

현재 `.worktrees/macos-ai-quota`의 `feat/macos-ai-quota` 브랜치에는 별도 macOS 구현이 일부 존재한다. 기존 구현과 미커밋 작업은 보존한다. 구현·시험의 세부 상태는 해당 작업 트리의 `macos/Docs/implementation-status.ko.md`에 기록돼 있으며, 이 계획의 기능 목록 자체를 구현 완료 증거로 취급하지 않는다.
...
- 모바일 기준: `artifacts/release-1.2.2-55/source-manifest.json`. 2026-09-17 재검토에서 기록된 718개 파일을 현재 파일과 SHA-256 대조해 변경·누락 0개를 확인했다.
...
저장소 배치는 별도 macOS 작업 트리 안의 `macos/`를 사용한다. 이미 있는 `.worktrees/macos-ai-quota`를 보존하고 Android 원본 작업 트리와 분리한다. Android의 기능 계약·익명화된 응답 fixture와 수정 이력을 함께 비교하되, 해당 브랜치에 체크아웃된 과거 Android 코드 대신 위 55 보관본을 기준으로 삼는다. 추후 별도 저장소로 분리할 수 있도록 Gradle 및 Android 자원에 대한 빌드 의존성은 만들지 않는다.
...
macos/
  ...
  Packages/AIQuotaAuth/          # Keychain, 격리 웹 세션, 로그인·갱신 수명주기
...
2026-09-17 확인한 작업 트리 기록에는 코어·인증 저장소·수집기·앱 화면·위젯 확장·고정 패널의 일부 구현과 합성 응답 시험 결과가 있다. 이 기록을 출발점으로 사용하되 이번 계획 검토에서 그 시험을 재실행한 것으로 보고하지 않는다.

1. **기존 구현과 기능 계약 대조:** 10개 제공자마다 연결 UI, 계정 확인, 실제 수집, 세션 갱신, 메뉴 표시, 위젯 표시, 알림의 상태를 별도 기록한다. 수집 파서가 있다는 이유로 로그인과 장기 수집까지 완료로 표시하지 않는다.
2. **인증의 남은 경로 완성:** Copilot·Antigravity의 연결/갱신 UI, GLM의 현재 모바일 연결 방식과의 차이, 외부 브라우저/패스키 제약을 먼저 정리한다. Claude 2계정과 Codex 2계정의 실제 동시 수집을 우선 검증한다.
3. **한 계정의 데이터를 끝까지 검증:** 실제 응답 → 계정 저장소 → 메뉴 막대 → 제공자/대시보드/배터리 위젯 → 알림까지 같은 계정과 지표가 유지되는지 확인한다. 메뉴 막대 실제 클릭, 위젯 갤러리 등록과 독립 구성, 권한 거부/취소도 포함한다.
4. **자원 사용과 업데이트 확인:** 정상 60초 수집을 유지한 채 CPU·메모리·요청량을 측정하고, 72시간 관찰 및 잠자기/네트워크 복구/재부팅/버전 업데이트로 세션과 알림 상태의 유지 여부를 확인한다.
5. **서명·배포:** Apple 서명 구성을 준비해 호스트와 WidgetKit 확장의 App Group 접근을 확인한 뒤 공증 DMG와 업데이트 경로를 완성한다. 서명 없는 빌드 성공을 설치·위젯·배포 성공으로 대신하지 않는다.

개발 우선순위는 인증과 데이터 정확성 → 메뉴 막대 → 계정별 위젯 → 장기 유지와 자원 최적화 → 배포다. 이미 구현된 부분은 재작성보다 검증과 부족한 연결을 보완한다.
